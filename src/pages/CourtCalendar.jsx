...
export default function CourtCalendar() {
  ...
  const myHearings = useMemo(() => {
    if (role === "member" || role === "client") {
      const myCaseIds = new Set(
        cases
          .filter((c) => String(c.member_id || "").toLowerCase() === String(memberId).toLowerCase())
          .map((c) => c.id)
      );
      return hearings.filter((h) => myCaseIds.has(h.caseId));
    }
    return hearings;
  }, [hearings, cases, role, memberId]);
  ...
  const stats = useMemo(() => {
    ...
    const todayCount = myHearings.filter((h) => h.hearingDate === todayStr).length;
    const upcomingCount = myHearings.filter((h) => h.hearingDate > todayStr).length;
    const totalHearings = myHearings.length;
