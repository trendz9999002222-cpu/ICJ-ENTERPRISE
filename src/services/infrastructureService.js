...
export const PROVIDER_CATALOG = [
  ...
  },
  {
    ...
    fields: [
      { key: "provider",  label: "Provider",    type: "select",   options: ["2Factor","Twilio","MSG91","Textlocal","Amazon SNS"], secret: false },
...
export const InfraService = {
  ...
  // Simulate or perform connection test
  async testConnection(providerId) {
    ...
    // If 2Factor is selected under SMS, run a real verification check if apiKey is present
    if (providerId === "sms" && String(config.provider).toLowerCase() === "2factor" && config.apiKey && status === "configured") {
      try {
        const url = `https://2factor.in/API/V1/${config.apiKey}/SMS/BAL/SMS`;
        const res = await fetch(url);
        const data = await res.json();
        if (data.Status === "Success") {
          result = {
            status: "connected",
            message: `2Factor SMS API key verified successfully. Balance: ${data.Details || "N/A"}.`,
            testedAt: new Date().toISOString(),
            note: "Real-time API key verification completed.",
          };
        } else {
          result = {
            status: "error",
            message: `2Factor API verification failed: ${data.Details || "Invalid API Key"}`,
            testedAt: new Date().toISOString(),
            note: "Real-time API verification failed.",
          };
        }
      } catch (err) {
        result = {
          status: "error",
          message: `Network/API connection error: ${err.message}`,
          testedAt: new Date().toISOString(),
          note: "Failed to reach 2Factor servers.",
        };
      }
    }
